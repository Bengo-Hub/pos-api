...
// for branded document numbering (order, pos_return, pos_reversal, repair_job — no
// separate receipt type; a receipt always carries its order's own number). Mirrors
// inventory/treasury-api's DocumentSequence; the service layer increments via
// optimistic CAS. Platform default is PURE NUMERIC (empty prefix + empty
// date_format), tenants opt into a prefixed/dated style in Settings.
...
func (DocumentSequence) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("doc_type").NotEmpty().Comment("order, pos_return, pos_reversal, repair_job, ..."),
