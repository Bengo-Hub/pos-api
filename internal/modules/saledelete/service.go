...
func (s *Service) Delete(ctx context.Context, tenantID uuid.UUID, req Request) (*Result, error) {
	...
	if hasHistory, err := orders.HasCorrectionHistory(ctx, s.client, tenantID, order.ID); err != nil {
	...
	fiscalized, invoiceID, err := orders.IsFiscalized(ctx, s.treasuryClient, req.TenantSlug, order.ID)
	if err != nil {
