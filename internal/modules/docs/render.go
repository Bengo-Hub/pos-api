...
// reportCtx threads the painter, report and vertical cursor through the render pipeline. pageH is
// the CURRENT report's actual physical page height (297mm portrait / 210mm landscape) — never the
// stale package-level "pageH" the whole break-check logic used to assume unconditionally.
...
func (rc *reportCtx) ensure(h float64) {
	if rc.y+h > rc.pageH-bottomMg-6.0 {
		rc.newPage()
	}
}
...
func (rc *reportCtx) drawTable(s Section) {
	...
	for idx, row := range s.Rows {
		if rc.y+rowH > rc.pageH-bottomMg-6.0 {
			rc.newPage()
			drawHeaderRow()
		}
		if idx%2 == 1 {
	...
	if len(s.Total) > 0 {
		if rc.y+rowH+1 > rc.pageH-bottomMg-6.0 {
			rc.newPage()
			drawHeaderRow()
		}
...
func (rc *reportCtx) drawKeyValue(s Section) {
	...
	for _, kv := range s.Pairs {
		if y+rowH > rc.pageH-bottomMg-6.0 {
...
func (rc *reportCtx) drawFooter() {
	...
	note := rc.r.Footer
	if note == "" {
		note = "Generated by Codevertex POS · " + formatDateTime(rc.r.GeneratedAt)
	}
	fy := rc.pageH - bottomMg + 1.0
	...
	// Platform-owner advertisement — platform default ON, per-tenant opt-out — centered under
	// the generated-by line.
	if rc.r.ProviderFooterEnabled {
		w := p.rightX - p.leftX
